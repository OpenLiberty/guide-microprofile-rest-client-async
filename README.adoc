...
This application uses the Apache Kafka and ZooKeeper services to maintain the distributed job results and system status.
These topics will not be covered in this guide. 
...
You can use `curl -X POST http://localhost:8080/api/jobs` command if available on the system to create a job. Postman application can also be used. The requests have some delay for the job results.

The all completed jobs JSON output has `averageResult` that is the average sleep time of all the jobs, a count for the number of jobs and the list of the jobs. Each job JSON output has a job ID and a sleep time as the result for this job.
