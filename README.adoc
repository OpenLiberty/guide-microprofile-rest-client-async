...
Asynchronous programming can be thought of as a take-out restaurant. When you join one of the lines to order at the register, you tell them your order, you pay, and you'll usually get some sort of order number. At this point, you’ll leave the line and wait 5 - 10 minutes off to the side for the cooks to prepare your food. Once your order is ready, your number will be called and you can pick up your food and your request for food has been fulfilled. In a synchronous model, you would place your order and continue waiting in line at the counter for your food to be prepared therefore blocking other customers from proceeding and placing their order.

Asynchronous methods are methods that allow you to perform a lengthy operation such as IO asynchronously. This means that the IO occurs in the background and the caller is notified via a callback to continue with their computation once it’s complete. The benefit is that this frees up the original thread to handle other work rather than waiting on IO to complete. Going back to the restaurant analogy, calling the `prepareFood()` method asynchronously frees up the person at the register. Since they're free, the person can take more orders from their line as they've been freed up from the responsibility of serving the previous person until their order is ready.

In the context of MicroProfile REST client, making HTTP requests can be a time consuming process. There might be slowness in the network, or maybe the upstream service is overwhelmed and can't respond in a timely manner. These lengthy operations can block the execution of your thread when it might be used instead to get other work done.
...
The only change is that the [hotspot=21 file=0]`getProperties()` method now returns `CompletionStage<Properties>` instead of `Properties`. This change makes the method asynchronous. Since the method now has a return type of `CompletionStage<Properties>`, you will not be able to directly manipulate the `Properties` object. As you will see in the next section, you will be able to indirectly use the `Properties` object by chaining callbacks.

== Responding asynchronously to an HTTP request

JAX-RS resources can also have asynchronous methods. So instead of returning a `Response` type you may return a `CompletionStage<Response>` type. Completion stages can be chained together using the [hotspot=40-43 file=0]`thenApplyAsync()` method, and exceptions can be handled using the [hotspot=44-50 file=0]`exceptionally()` method.
...
Similar to the synchronous approach, if we successfully get the properties from the server running at a given host name then the resource will respond with an HTTP status of 200 and the body will contain the system properties. If an exception occurs the response will have a 404 status and a body containing an error message. Finally, return the `CompletionStage<Response>` you built up using the [hotspot=40-43 file=0]`thenApplyAsync()` and [hotspot=44-50 file=0]`exceptionally()` methods.

A completion stage represents a unit of computation once that computation completes it can either be finished or chained with more completion stages using [hotspot=40-43 file=0]`thenApplyAsync()` to perform more computations. Exceptions can be handled in a callback provided to the [hotspot=44-50 file=0]`exceptionally()` method, it functionally behaves similar to a catch block. When you return a completion stage in the resource, it doesn’t necessarily mean that the computation has completed and the response has been built. JAX-RS will respond to the user once the `CompletionStage` has completed.
