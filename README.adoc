...
When a client returns, the [hotspot=thenAcceptAsync file=0]`thenAcceptAsync()` method will process the returned data with the `CompletionStage<Properties>` interface.
