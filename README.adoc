...
Completion stages can be chained together by using the [hotspot=thenApplyAsync file=0]`thenApplyAsync()` method.
...
If properties are received as a `List<String>` they are added to a `Holder` using the [hotspot=thenApplyAsync file=0]`thenApplyAsync()` method.
A [hotspot=countdown file=0]`CountDownLatch` keeps track of our asynchronous requests to know when they're all finished.
...
* The [hotspot=getSystems file=0]`testGetSystems()` test case verifies that the `gateway` service communicates with a backend service to get all system info.
* The [hotspot=badSystem file=0]`testBadSystem()` test case verifies that the `gateway` service properly handles a bad system request.
* The [hotspot=osInfo file=0]`testOsInfo()` test case verifies that the `gateway` service asynchronously communicates with the backend service.
