...
== Client Definition - Changing the template interface to use asynchronous methods
...
== Service Definition - Responding asynchronously to an HTTP request
...
Before any jobs have been created, the JSON output will be this `{"count":0,"results":[]}`. The `count` attribute is 0 because there are no jobs yet, and the `results` attribute is empty for the same reason.
