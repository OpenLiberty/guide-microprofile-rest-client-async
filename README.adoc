...
This returns a `CompletionStage` interface which we process asynchronously in the [hotspot=thenAcceptAsync file=0]`thenAcceptAsync` method. 

In this method we use a [hotspot=countdown1 hotspot=countdown2 hotspot=countdown3 file=0]`CountDownLatch` to keep track of our asynchronous requests. 
