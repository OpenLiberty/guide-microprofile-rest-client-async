...
Then it loops through each hostname and calls [hotspot=getSystem file=0]`getSystem()` on each one. 
This returns a `CompletionStage` interface which is processed asynchronously in the [hotspot=thenAcceptAsync file=0]`thenAcceptAsync()` method. 
...
The `CountDownLatch` being at zero indicates that all requests are complete.
By using the [hotspot=await file=0]`await()` method of the `CountDownLatch`, the program waits for all the requests to be complete before it continues.
