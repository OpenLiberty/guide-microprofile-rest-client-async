...
JAX-RS resources can also have asynchronous methods. So instead of returning a `Response` type you may return a `CompletionStage<Response>` type. Completion stages can be chained together using the [hotspot=42-45 file=0]`thenApplyAsync()` method, and exceptions can be handled using the [hotspot=46-52 file=0]`exceptionally()` method.
