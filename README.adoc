...
To reduce the processing time, you will update the `/query/systemLoad` endpoint to send the requests asynchronously. 
Multiple client requests will be sent synchronously in a loop.
Since asynchronous calls do not block the program, the endpoint need to ensure that all calls are completed and all returned data is processed before proceeding.
