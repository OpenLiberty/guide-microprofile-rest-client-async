...
the `gateway` microservice is a https://microservices.io/patterns/apigateway.html#variation-backends-for-frontends[backend for front end^].
It communicates with the backend `job` and `inventory` microservices on the caller's behalf.

image::guide-{projectid}/assets/reactive-inventory-system.png[]
...
Next, use the provided script to start the application in docker containers. The script creates a network for the containers to communicate with eachother. It also spawns containers for kafka, zookeeper, and all of the microservices in the project.
...
A few tests are included for you to test the basic functionality of the `gateway` microservice. If a test failure occurs, then you might have introduced a bug into the code.
