...
Change the return type to `CompletionStage<Response>` and add the annotation `@Asynchronous`.
These changes make the method asynchronous.
Since the method now has the return type of `CompletionStage<Response>`, you aren't able to directly manipulate the inner `Response`. 
...
In the [hotspot=systemLoad file=0]`/query/systemLoad` endpoint multiple systems are read and compared at once to 
