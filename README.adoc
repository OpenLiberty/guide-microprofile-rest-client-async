...
:page-related-guides: ['microprofile-rest-client', 'microprofile-reactive-messaging', 'reactive-service-testing', 'reactive-messaging-sse', 'microprofile-reactive-messaging-acknowledgement']
...
https://microservices.io/patterns/apigateway.html#variation-backends-for-frontends[backend for frontend^] service.
...
First, you will place order requests and observe that the requests are processed synchronously.
To convert the application to use an asynchronous model, you will modify the `OpenLibertyCafe` service.
The MicroProfile Rest Client and JAX-RS resources in the `OpenLibertyCafe` microservice will be updated
to use asynchronous methods and asynchronously handle HTTP requests.
...
After you are done checking out the application, stop the application using the provided script:
...
This asynchronous approach also uses a [hotspot=countDownLatch file=0]`CountDownLatch`
that allows a thread to wait until other threads complete a set of operations.
The [hotspot=countDownLatch file=0]`CountDownLatch` is initialized with a count,
which is equal to the total number of food orders and beverage orders.
In the [hotspot=thenAcceptAsync1 hotspot=thenAcceptAsync2 file=0]`thenAcceptAsync()` method, every time an order is processed,
the [hotspot=countDownLatch file=0]`CountDownLatch` decrements its count by calling the 
[hotspot=countDown1 hotspot=countDown2 file=0]`countdownLatch.countDown()` method.
The thread calling the [hotspot=await file=0]`countdownLatch.await()` method is blocked until the count reaches zero.
So, the [hotspot=await file=0]`countdownLatch.await()` method waits until all of the asynchronous 
[hotspot=thenAcceptAsync1 hotspot=thenAcceptAsync2 file=0]`orderClient.createOrder(order)` method executions are completed,
and then the thread is free to return the list of order IDs.
...
To build your asynchronous application, repackage the `OpenLibertyCafe` microservice:

[role='command']
```
mvn -pl openLibertyCafe package
```

Run the following command to rebuild the Docker image for the `OpenLibertyCafe` microservice:
