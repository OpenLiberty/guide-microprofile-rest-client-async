...
You will learn how to build a MicroProfile Rest Client to access remote RESTful services using asynchronous method calls. You'll also update a template interface that maps to the remote service that you want to call. You'll change the template interface to use a `CompletionStage` return type, which makes the method asynchronous. A `CompletionStage` is an interface that will allow you to use the result of your remote service call at some point in the future.
