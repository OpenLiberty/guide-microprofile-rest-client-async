...
== Updating template interface of REST client to use asynchronous methods
...
== Updating REST resource to handle HTTP requests asynchronously
...
If no jobs have been created, the JSON output will be this `{"count":0,"results":[]}`. The `count` attribute is 0 because there are no jobs yet, and the `results` attribute is empty for the same reason.
