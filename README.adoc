...
You will learn how to build a MicroProfile Rest Client to access remote RESTful services using asynchronous method calls. You'll update a template interface that maps to the remote service that you want to call. You'll change it to use a `CompletionStage` return type making the method asynchronous. A `CompletionStage` is a class that will allow you to use the result of your remote service call at some point in the future.
...
the `bff` service communicates with the `job`
