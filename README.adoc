...
This application will get all the hostnames by calling [hotspot=getSystems file=0]`getSystems`.
...
When it gets to zero, we know that all our requests are complete.
Then by using the [hotspot=await file=0]`await` method of the `CountDownLatch`, the program waits for all the requests to be complete before it continues.
