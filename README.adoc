...
You will learn how to build a MicroProfile Rest Client to access remote RESTful services using asynchronous method calls. You'll also update a template interface that maps to the remote service that you want to call. You'll change the template interface to use a `CompletionStage` return type, which makes the method asynchronous. A `CompletionStage` interface will allow you to use the result of your remote service call.
...
Asynchronous methods allow lengthy operations, such as input/output (I/O) asynchronously, to perform asynchronously. As the I/O occurs in the background, a callback notifies the caller to continue its computation after the original request is complete. As a result, the original thread frees up so it can handle other work rather than wait for the I/O to complete. Revisiting the restaurant analogy, food is prepared asynchronously in the kitchen and your waiter is freed up to attend to other tables. Since they’re free, the waiter can take more orders from other tables, or serve food to other tables.

In the context of REST clients, making HTTP requests can be a time-consuming process. The network might be slow, or maybe the upstream service is overwhelmed and can't respond quickly. These lengthy operations can block the execution of your thread when it's in use and prevent other work from being completed.
...
First, make the job client interface methods asynchronous.
...
JAX-RS resources can also have asynchronous methods. So instead of returning a `JobsListModel` type, you can return a `CompletionStage<JobsListModel>` type. Completion stages can be chained together by using the [hotspot=thenApplyAsync file=0]`thenApplyAsync()` method.
...
Similar to the synchronous approach, if completed jobs are successfully obtained from the `job` microservice, the resource responds with an HTTP status of 200 and the body will contain a list of jobs. Finally, return the `CompletionStage<JobsListModel>` result you built by using the [hotspot=thenApplyAsync file=0]`thenApplyAsync()` method.
...
The `gateway`, `job`, `system`, and `inventory` microservices will be built in a Docker container. You can learn more about containerizing microservices with Docker in the https://openliberty.io/guides/containerize.html[Containerizing microservices^] guide.
...
The completed jobs JSON output has an `averageResult` attribute that is the average sleep time of all the jobs, a count for the number of jobs, and the list of the jobs. The JSON output for each job has a job ID and a sleep time as the result for the job.
...
* The [hotspot=testGetJobs file=0]`testGetJobs()` test case verifies that the gateway service communicates with an upstream service to get all jobs and transforms them to a `JobListModel` type.
