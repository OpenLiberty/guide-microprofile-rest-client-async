...
Each hostname is asynchronously processed using the method [hotspot=getSystem file=0]`getSystem()` and the method [hotspot=thenAcceptAsync file=0]`thenAcceptAsync()` from the `CompletionStage` interface.
